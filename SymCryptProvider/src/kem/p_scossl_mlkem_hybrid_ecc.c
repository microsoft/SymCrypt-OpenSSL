...
_Use_decl_annotations_
void p_scossl_mlkem_hybrid_ecc_free_ctx(SCOSSL_ECC_KEY_CTX *keyCtx)
{
    if (keyCtx == NULL)
        return;
    if (keyCtx->key != NULL)
    ...
    // Hybrid keys never enable KeysInUse, but the lock field may exist in
    // the struct; freeing NULL is safe.
...
_Use_decl_annotations_
SCOSSL_ECC_KEY_CTX *p_scossl_mlkem_hybrid_ecc_dup_ctx(const SCOSSL_ECC_KEY_CTX *keyCtx, int selection)
{
    PBYTE pbData = NULL;
...
cleanup:
    ...
    if (!success)
    {
        p_scossl_mlkem_hybrid_ecc_free_ctx(copyCtx);
        copyCtx = NULL;
    }
...
_Use_decl_annotations_
SCOSSL_STATUS p_scossl_mlkem_hybrid_ecc_set_group(SCOSSL_ECC_KEY_CTX *keyCtx, int nid)
{
...
_Use_decl_annotations_
SCOSSL_STATUS p_scossl_mlkem_hybrid_ecc_gen(SCOSSL_ECC_KEY_CTX *keyCtx)
{
    SYMCRYPT_ERROR scError = SYMCRYPT_NO_ERROR;
...
static SCOSSL_STATUS p_scossl_mlkem_hybrid_ecc_get_encoded_public_key(_In_ const SCOSSL_ECC_KEY_CTX *keyCtx,
                                                                      _Inout_ PBYTE *ppbPublicKey, _Inout_ SIZE_T *pcbPublicKey)
{
    ...
    else
    {
        ...
        // Allocate one extra byte for the point conversion format prefix
    ...
    if (!keyCtx->isX25519)
    {
        // Hybrid always uses POINT_CONVERSION_UNCOMPRESSED
        pbEncodedPublicKey[0] = POINT_CONVERSION_UNCOMPRESSED;
    }
...
static SCOSSL_STATUS p_scossl_mlkem_hybrid_ecc_get_private_key(_In_ const SCOSSL_ECC_KEY_CTX *keyCtx,
                                                               _Inout_ PBYTE *ppbPrivateKey, _Inout_ SIZE_T *pcbPrivateKey)
{
    PBYTE pbPrivateKey = NULL;
    SIZE_T cbPrivateKey;
...
_Use_decl_annotations_
SCOSSL_STATUS p_scossl_mlkem_hybrid_ecc_get_encoded_key(const SCOSSL_ECC_KEY_CTX *keyCtx, int selection,
                                                        PBYTE *ppbKey, SIZE_T *pcbKey)
{
    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0)
    {
        return p_scossl_mlkem_hybrid_ecc_get_private_key(keyCtx, ppbKey, pcbKey);
    }
    return p_scossl_mlkem_hybrid_ecc_get_encoded_public_key(keyCtx, ppbKey, pcbKey);
}

_Use_decl_annotations_
SCOSSL_STATUS p_scossl_mlkem_hybrid_ecc_set_encoded_key(SCOSSL_ECC_KEY_CTX *keyCtx,
...
cleanup:
    if (ret != SCOSSL_SUCCESS && keyCtx->key != NULL)
    {
        SymCryptEckeyFree(keyCtx->key);
        keyCtx->key = NULL;
    }
