...
static SCOSSL_STATUS scossl_aes_cbc_cipher(_In_ SYMCRYPT_AES_EXPANDED_KEY *key, _Inout_updates_(SYMCRYPT_AES_BLOCK_SIZE) PBYTE pbChainingValue, 
                                           int encrypt, _Out_writes_bytes_(inl) unsigned char *out,
                                           _In_reads_bytes_(inl) const unsigned char *in, size_t inl)
{
    if (encrypt)
    {
...
SCOSSL_CBC_CTX *p_scossl_aes_cbc_newctx_internal(size_t keylen)
{
    SCOSSL_CBC_CTX *ctx = OPENSSL_zalloc(sizeof(SCOSSL_CBC_CTX));
    if (ctx != NULL)
    {
        ctx->keylen = keylen;
        ctx->pad = 1;
        ctx->cipher = &scossl_aes_cbc_cipher;
    }

    return ctx;
}

void p_scossl_aes_cbc_freectx(SCOSSL_CBC_CTX *ctx)
{
    OPENSSL_clear_free(ctx, sizeof(SCOSSL_CBC_CTX));
}

SCOSSL_CBC_CTX *p_scossl_aes_cbc_dupctx(SCOSSL_CBC_CTX *ctx)
{
    SCOSSL_CBC_CTX *copy_ctx = OPENSSL_malloc(sizeof(SCOSSL_CBC_CTX));
    if (copy_ctx != NULL)
    {
        memcpy(copy_ctx, ctx, sizeof(SCOSSL_CBC_CTX));
        SymCryptAesKeyCopy(&ctx->key, &copy_ctx->key);
    }
    return copy_ctx;
}

SCOSSL_STATUS p_scossl_aes_cbc_init_internal(SCOSSL_CBC_CTX *ctx,
                                             ...
                                             const OSSL_PARAM params[])
{
    if (key != NULL)
    {
        ...
        {
            SCOSSL_LOG_INFO(SCOSSL_ERR_F_AES_CCM_CIPHER, SCOSSL_ERR_R_SYMCRYPT_FAILURE, "Key len %d\n", keylen);
    ...
    return p_scossl_aes_cbc_set_ctx_params(ctx, params);
}

SCOSSL_STATUS p_scossl_aes_cbc_encrypt_init(SCOSSL_CBC_CTX *ctx,
                                            const unsigned char *key, size_t keylen,
                                            const unsigned char *iv, size_t ivlen,
                                            const OSSL_PARAM params[])
{
    ...
    return p_scossl_aes_cbc_init_internal(ctx, key, keylen, iv, ivlen, params);
}

SCOSSL_STATUS p_scossl_aes_cbc_decrypt_init(SCOSSL_CBC_CTX *ctx,
                                            const unsigned char *key, size_t keylen,
                                            const unsigned char *iv, size_t ivlen,
                                            const OSSL_PARAM params[])
{
...
SCOSSL_STATUS p_scossl_aes_cbc_update(SCOSSL_CBC_CTX *ctx,
                                      ...
                                      const unsigned char *in, size_t inl)
{
    ...
    if (ctx->cbBuf == SYMCRYPT_AES_BLOCK_SIZE)
    {
        ...
        if (!ctx->cipher(&ctx->key, ctx->pbChainingValue, ctx->encrypt, out, ctx->buf, ctx->cbBuf))
        {
            ERR_raise(ERR_LIB_PROV, PROV_R_CIPHER_OPERATION_FAILED);
            return SCOSSL_FAILURE;
        }
    ...
    if (cBlocksRemaining > 0)
    {
        ...
        if (!ctx->cipher(&ctx->key, ctx->pbChainingValue, ctx->encrypt, out, in, cBlocksRemaining * SYMCRYPT_AES_BLOCK_SIZE))
        {
            ERR_raise(ERR_LIB_PROV, PROV_R_CIPHER_OPERATION_FAILED);
            return SCOSSL_FAILURE;
        }
...
SCOSSL_STATUS p_scossl_aes_cbc_final(SCOSSL_CBC_CTX *ctx,
                                     unsigned char *out, size_t *outl, size_t outsize)
{
    if (ctx->encrypt)
    {
...
SCOSSL_STATUS p_scossl_aes_cbc_cipher(SCOSSL_CBC_CTX *ctx,
                                      unsigned char *out, size_t *outl, size_t outsize,
                                      const unsigned char *in, size_t inl)
{
...
IMPLEMENT_SCOSSL_CIPHER_FUNCTIONS(aes, 128, cbc, 0)
IMPLEMENT_SCOSSL_CIPHER_FUNCTIONS(aes, 192, cbc, 0)
IMPLEMENT_SCOSSL_CIPHER_FUNCTIONS(aes, 256, cbc, 0)

/*
extern const OSSL_DISPATCH p_scossl_aes128cbc_functions[]; IMPLEMENT_generic_cipher
extern const OSSL_DISPATCH p_scossl_aes192cbc_functions[]; IMPLEMENT_generic_cipher
extern const OSSL_DISPATCH p_scossl_aes256cbc_functions[]; IMPLEMENT_generic_cipher

extern const OSSL_DISPATCH p_scossl_aes128ecb_functions[]; IMPLEMENT_generic_cipher
extern const OSSL_DISPATCH p_scossl_aes192ecb_functions[]; IMPLEMENT_generic_cipher
extern const OSSL_DISPATCH p_scossl_aes256ecb_functions[]; IMPLEMENT_generic_cipher
