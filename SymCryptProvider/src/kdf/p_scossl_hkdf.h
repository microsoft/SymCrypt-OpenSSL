...
SCOSSL_STATUS p_scossl_tls13kdf_derive(_In_ SCOSSL_PROV_TLS13_KDF_CTX *ctx,
    _Out_writes_bytes_(keylen) unsigned char *key, size_t keylen,
    _In_ const OSSL_PARAM params[]);
...
SCOSSL_STATUS p_scossl_tls13kdf_set_ctx_params(_Inout_ SCOSSL_PROV_TLS13_KDF_CTX *ctx, const _In_ OSSL_PARAM params[]);
