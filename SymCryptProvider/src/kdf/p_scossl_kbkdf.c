...
{
    SCOSSL_PROV_KBKDF_CTX *copyCtx;

    if (ctx == NULL)
        return NULL;

    copyCtx = OPENSSL_malloc(sizeof(SCOSSL_PROV_KBKDF_CTX));
    if (copyCtx != NULL)
    {
        *copyCtx = *ctx;
                                                ...
                                                _Out_writes_bytes_(keylen) unsigned char *key, size_t keylen)
{
    ...
    {
        SCOSSL_PROV_LOG_SYMCRYPT_ERROR("SymCryptKmacXXXExpandKey failed", scError);
        goto cleanup;
    }
                                           ...
                                           _In_ const OSSL_PARAM params[])
{
    ...
    if (scError != SYMCRYPT_NO_ERROR)
    {
        SCOSSL_PROV_LOG_SYMCRYPT_ERROR("SymCryptSp800_108 failed", scError);
        return SCOSSL_FAILURE;
    }
